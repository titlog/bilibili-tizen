...
| `卡住 15 秒无错误也无进展，当作断连走重载阶梯` | 无错误型卡死的看门狗在**行动**（2026-08-06 起）。当天 13:33 实录：cosov 403 被正确拉黑后，唯一剩下的主机既不给字节也不报错，黑屏两分多钟而失败阶梯一步没走——阶梯全部由错误事件触发，没有错误就没有自救，观众只能退出重开（那等于手动轮转镜像）。现在缓冲末端 15 秒零进展就按解码失败阶梯处理（轮转重载 → 换族 → 压档，共用同一份按事故计的预算），阶梯用尽才 emit error 交给原地重启/有声退出。看到这行说明卡死是「掐线不报错」型，不是解码问题 |
