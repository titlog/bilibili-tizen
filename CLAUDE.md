...
需要 CSRF 令牌 `bili_jct`，TV 登录路径给得出，网页扫码兜底那条给不出 —— 那种
账号会静默地不上报，所以 `reportProgress()` 在日志里说一次。节奏是**每 30 秒
一次**，外加停止播放时一次、播完一次（`progress=-1`）。官方网页播放器是 15 秒；
这里故意慢一倍，因为这是真人的账号，快一倍只换来「新鲜到 15 秒」而不是 30 秒。
