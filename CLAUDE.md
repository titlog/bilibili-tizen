...
| `沿用 N 分钟前的教训：编码从 … 出发，坏主机 X，坏文件 Y` / `这条路稳定播了一分钟，记为本视频的教训（…）` / `上次渐进式在这个视频上败过，打平直接走 DASH` | 按视频的教训存储（2026-08-09 起，`bili.lessons.v1`，设备级）。重进病视频不再从头重学：赢的编码族、坏主机、坏文件、渐进式败绩都按 cid 记住，**六小时过期**——这是设计不是省事，按文件的天气隔晚必翻（08-06 av01 是病、08-09 av01 是药），昨天的教训是谎言。编码教训是**软偏好**（领跑族序、绝不锁死，族不在了自动落回默认序）；「稳定播一分钟」和预算回血是同一个信号。记录上限 100 条，最旧先淘汰 |
