...
| `到画面 playurl=… teardown=… manifest=… loaded=… playing=…` | 启动时间的分解，毫秒，从按键算起。**动启动路径之前先读它**；一个视频只打一行（`playing` 事件在每次拖动后都会再触发，而刻度只记第一次，以前同一份数字会重复打四五遍）。**走了 `downgrade()` 兜底的视频上这行不可信**：刻度一个视频只记一次，于是 `playing=` 记的是 AVPlay 那个「准备成功随即失败」的假事件，而真正的画面是几秒后 DASH 给的 —— 这条路上要看 `编码`/`画质切到` 的时间戳 |
...
- **有的稿件 AVPlay 必然打不开，而地址是好的。** `BV1HF411s7hK` 每一次都是：
  ...
  「一个错误码可能意味着好几件事」那一条。
- **播放头会自己往回跳。** 观测到两次：`1021.2s → 13.8s` 和 `383.5s → 357.7s`，
  期间没人碰遥控器，Shaka 自己的 `bufferingTime` 一动不动（所以不是卡顿）。**它
  同时是「续播点丢了」的原因**：跳到 30 秒以内之后 `Resume.record()` 照常写入，
  `resumable` 为 false，存着的位置被清成 0。`跳转` 那行日志就是为它加的，会指认
  发起方；加上之后**尚未复现**，所以现在只能说「没再出现」，不能说修好了。
