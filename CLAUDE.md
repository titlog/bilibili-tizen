...
| 1080p 编码 | 桌面自动档选了 **av01** | hev1 优先、AV1 排最后 —— TV SoC 软解顾虑，刻意差异 |
...
应对是给失败阶梯加了 AV1 这一档（hev1 → avc1 → **av01 同画质** → 压 720p），
日志认 `换 AV1 的同画质`。**AV1 是不是硬解仍是悬案** —— 这一档真正打起来时，
看 `卡住/恢复播放` 行里的 `丢帧` 计数，它是唯一的裁判；软解丢帧的话下面还有
720p 兜底。这种按文件的歧视也是按稿件来的：P18/P19 的 hev1 一整晚都好好的。
