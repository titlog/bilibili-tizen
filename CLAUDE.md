...
| 日志行 | 回答什么 |
|---|---|
| `BV1xxx 标题 — requesting qn=80` | 是哪个视频 —— **每条播放日志都带 bvid**，否则「有个视频打不开」无从追查 |
| `dash offers qn=80 accept=…` / `progressive gave qn=…` | 两种形式各自能给什么，路由为什么这么选 |
| `编码 hev1` | 这一条流实际用的编码族；后面跟「（指定）」说明是失败退回来的 |
| `到画面 playurl=… teardown=… manifest=… loaded=… playing=…` | 启动时间的分解，毫秒，从按键算起。**动启动路径之前先读它**；一个视频只打一行（`playing` 事件在每次拖动后都会再触发，而刻度只记第一次，以前同一份数字会重复打四五遍） |
| `画质切到 1920x1080 (1897 kbps)` | ABR 切档；向上切也会触发，不是故障 |
| `卡住 t=… ahead=… 估算带宽=… 已卡=… 丢帧=…` | 卡顿现场，取自 Shaka 自己的统计 |
