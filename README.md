...
- **这个固件的 widget 根本发不出 `Cookie` 头** —— Chromium 120 对 forbidden header
  的 `setRequestHeader` 静默忽略,引擎的全局 cookie jar 也灌不进(跨域封锁)。所以
  登录态会话改走 `access_key`(拼进 query,没人剥得掉)。这条推翻了文档里断言了七天
  的「这固件允许 widget 设 Cookie 头」—— 那次测量是在只有一个账号时做的,而那时
  `Cookie` 头和 jar 必然指向同一个人,根本没有区分能力,直到第二个账号出现才露馅。
...
- **换 CDN 节点在这条链路上行不通。** 签名是绑主机的,八个候选里七个 403,唯一能
  用的那个慢三十倍。
- **同一稿件手机能放、电视不能,根因是 playurl 的 `platform`。** web 端点
  (`platform=pc`,客户端一直走的)铸的流令牌被 CDN 严格节点 403,app 端点
  (`app.bilibili.com/x/playurl`,`platform=android`)铸的被接受 —— 手机 App 就是
  app 端点客户端。`platform` 决定令牌强度,`access_key` 决定档位可见性(不带只给
  免登录低档),两样都要。撞 403 时改用 app 端点铸强令牌、自读文件头补 app 端点不给
  的 `SegmentBase`。完整排查(含两次把「天气」当诊断的错判与纠正)见
...
文档里还记着一套排查纪律:**说得通的机制不等于诊断**。一次次,一个错误被第一个能
自圆其说的机制解释、据此设计、然后发布出去,后来被测量推翻 —— 上面那两条各自就是
一次(「允许设 Cookie 头」、「稿件全 403 是天气」),而查清 `platform` 那一夜光
「天气」就误判了两回。每一次上一个解释都仍然成立,也仍然不是原因。**跨设备的差异,
要抓被质疑那台设备自己产出的证物;跨时刻的两个观测不能相减归因。**
